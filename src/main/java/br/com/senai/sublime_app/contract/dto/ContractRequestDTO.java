...
public record ContractRequestDTO(
        ...
        @NotNull(message = "O paciente titular é obrigatório.") Long patientId,

        // nullable: Pilates em Dupla / plano Familiar
        Long beneficiaryId,

        @NotNull(message = "A técnica âncora é obrigatória.") Long techniqueId,

        @NotNull(message = "O plano é obrigatório.") Long planId,

        @NotNull(message = "A frequência semanal é obrigatória.") @Min(value = 1, message = "A frequência semanal deve ser de no mínimo 1 sessão.") Integer weeklyFrequency,

        @NotNull(message = "A data de início é obrigatória.") LocalDate startDate,

        @NotNull(message = "A data de término é obrigatória.") LocalDate endDate,
